...
namespace GameEngine
{
    public class Level
    {
        ...
        internal void SetWallet(string filename, string walletAddress)
        {
            XmlDocument doc = new XmlDocument();
            doc.Load(filename);

            if (doc.DocumentElement.Attributes["wallet"] == null)
                doc.DocumentElement.Attributes.Append(doc.CreateAttribute("wallet"));
            doc.DocumentElement.SetAttribute("wallet", walletAddress);
            doc.Save(filename);
        }

        internal string GetWallet(string filename)
        {
            XmlDocument doc = new XmlDocument();
            doc.Load(filename);
